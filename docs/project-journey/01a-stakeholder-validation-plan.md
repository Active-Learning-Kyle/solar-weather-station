...
**Status:** Gates A–E teaching preview complete; real TA decision validation, site evidence, reference access, quotations and prototype performance remain open operational evidence
...
## Teaching-preview completion and real-evidence boundary
...
- [x] Tabletop decision method completed for teaching presentation.
  - VAL-004 records the instructor's `revise` decision. The [synthetic TA role-play and alignment](../../tests/tabletop-decision-pack/README.md) show the completed method but do not satisfy real verification T-082.
- [x] Matrix sensitivity and conditional strategy completed for teaching presentation.
  - VAL-005 plus the [synthetic Gate E workshop](gate-e-synthetic-workshop.md) select Concept E conditionally. Real participant scoring, quotations/loan access and T-083 remain open before spending or deployment.
- [ ] Hong Kong/institutional cost and availability evidence added.
- [x] `01-problem.md`, `02-solution-landscape.md` and the A–E overview updated with findings.

Issue #3 may close for the documented solution-landscape study and teaching-preview scope. The unchecked items above remain required for a real purchase, site deployment or performance claim; closing the Issue does not waive them. Concept E is a conditional learning direction, not a locked product purchase or deployment authorisation.
