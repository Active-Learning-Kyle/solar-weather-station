...
[Solution-landscape Issue #3](https://github.com/heqihao522828-crypto/solar-weather-station/issues/3) is used read-only during the first formal GitHub active-learning exercise. It shows students that a useful Issue contains context, a definition of done, evidence increments and honest external blockers. Gate A now confirms the instructor's field-readiness decision need and claim boundaries; the candidate site, formal authority, TA workflow, reference access, thresholds and actual system performance remain unvalidated. The linked [validation plan](../docs/project-journey/01a-stakeholder-validation-plan.md) shows how evidence can validate one layer without making the whole problem “complete.”
