...
[Solution-landscape Issue #3](https://github.com/heqihao522828-crypto/solar-weather-station/issues/3) is used read-only during the first formal GitHub active-learning exercise. It shows students that a useful Issue contains context, a definition of done, evidence increments and honest external blockers. Gate A confirms the instructor's field-readiness decision need and claim boundaries; Gate B confirms a real TA's workflow needs. Gate C pre-walk requirements are drafted, while the exact site, formal authority, operational dry run, reference access, thresholds and actual system performance remain unvalidated. The linked [validation plan](../docs/project-journey/01a-stakeholder-validation-plan.md) shows how evidence can validate one layer without making the whole problem “complete.”

Start the completed teaching presentation with the [one-page Gates A–E overview](../docs/project-journey/00-gates-a-e-overview.md). It makes the evidence labels and final hybrid direction visible before students explore the detailed records.

For class discussion before real site access, use the [clearly labelled synthetic Gate C example](../tests/site-validation-pack/synthetic-completed-example.md). Students should identify which statements are fictional inputs, which conditions would change the design and why the scenario cannot be cited as real permission or test evidence.

For decision practice, use the [Gate D tabletop pack](../tests/tabletop-decision-pack/README.md). Students first make their own choice, then compare the instructor decision and clearly fictional TA role-play. The [Gate E synthetic workshop](../docs/project-journey/gate-e-synthetic-workshop.md) demonstrates why reliable-data and engineering-learning objectives support a hybrid reference-plus-student-prototype strategy rather than a sunk-cost `buy or build` binary.

There is no automatic “Gate F.” Gate E hands the team into the ordinary open-engineering cycle: build the selected direction, verify subsystems, integrate, run real field validation, reflect and release. New gates should be added only when a real high-risk or irreversible decision needs one.
