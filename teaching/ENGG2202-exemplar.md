...
For decision practice, use the [Gate D tabletop pack](../tests/tabletop-decision-pack/README.md). Students compare an instructor's `revise` decision with an independent TA response and diagnose whether disagreement comes from safety classification, continuation thresholds, data presentation or authority. The [Gate E provisional re-score](../docs/project-journey/gate-e-provisional-rescore.md) then demonstrates why reliable-data and engineering-learning objectives support a hybrid reference-plus-student-prototype strategy rather than a sunk-cost `buy or build` binary.
